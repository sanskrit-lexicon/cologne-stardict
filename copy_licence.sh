...
for Val in "${dictList[@]}"
do
	cp ../Cologne_localcopy/$Val/pywork/"$Val"header.xml output/licenses/"$Val"_license.xml
	cp ../Cologne_localcopy/$Val/pywork/"$Val"header.xml production/licenses/"$Val"_license.xml
done
